...
- 决策：
  ...
  - `pnpm deploy:preprod:mobile` 负责以 `VITE_API_BASE_URL=/api` 构建 `apps/mobile`，并把 `apps/mobile/dist` 同步到 `<PREPROD_REMOTE_SITE_DIR>`
  - `pnpm deploy:check:preprod` 必须同时探活公网 H5 首页入口资源与 API，避免后端已更新但根站点仍停留在旧静态产物
- 原因：`<PREPROD_PUBLIC_BASE_URL>/` 的页面效果来自 Nginx 静态目录，不来自 backend 进程；如果根部署命令只重启后端，用户看到的 H5 仍会是旧构建产物。
