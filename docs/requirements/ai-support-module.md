...
6. 单个访客每天最多发起 30 次 AI 提问；超出后不再请求第三方 AI，直接提示“今天的 AI 咨询次数已用完”并引导联系一对一客服。

## 业务知识上下文

Backend 发送给 AI 平台的 system message 需要携带项目内稳定事实，避免把通用客服回答误当成本项目能力：

- 预约进度：优先说明用户可从 `/customer/records` 的“预约记录”进入订单详情，或在提交预约后进入 `/customer/progress/:orderId` 查看；当前进度以 backend 返回的 `scrap_order.currentStatus` 和 `order_timeline` 为准，不允许 AI 自行推断订单状态。
- 报废补贴：按商务部等 8 部门《2026 年汽车以旧换新补贴实施细则》整理为只读政策上下文，包含车辆条件、补贴标准、材料时间、申领入口和“以地方最新通知为准”的风险提醒；AI 不承诺一定能领、具体金额或审核结果。
- 补贴政策来源：`https://www.mofcom.gov.cn/zcfb/gnmygl/art/2025/art_56cd7e5f1aae46178d6bade99d0a1a7a.html`
...
- `usage`
  - 便于后续统计成本
- `escalation`
  - 由 backend 返回升级一对一客服建议，减少前端写死业务规则
...
- 计数对象：当前以 `X-Visitor-Key` / `car.visitor.key` 作为“一个用户”的轻量识别口径。
- 前端本地：按自然日把有效 AI 提问次数写入 `localStorage`，达到 30 次后直接返回限额提示，不调用 `/app/support/chat/stream`。
- Backend 兜底：同样按访客和日期做日限额校验，超过 `AI_SUPPORT_DAILY_LIMIT` 后直接返回统一响应，不调用第三方 AI 供应商。
- 默认限额：`AI_SUPPORT_DAILY_LIMIT` 未配置时为 30；本地开发或测试可配置为 `0` 验证超限分支。
- 产品口径：不做“假装一直回复中”，避免用户误认为页面卡死；超限时明确提示并展示一对一客服入口。
...
5. 聊天页 DOM 采用“头部 / 独立滚动消息区 / 底部输入栏”三段式；页面根容器固定在 viewport 内并锁住 `body/html` 外层滚动，输入栏不跟随页面滚走；移动端通过 `visualViewport` 同步动态可视高度，减少键盘弹出时底部输入栏错位
