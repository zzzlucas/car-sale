...
- [ ] 在 `apps/admin-web` 增加高德 Key 用量记录与预估面板
  - 触发：当地图地址搜索、逆地理编码等链路开始稳定进入真实使用阶段，需要判断闲鱼 Key 池的消耗速度和兜底时机。
  - 价值：能直观看到每个 Key 的调用次数、成功率、近期趋势，并粗略预估剩余额度还能支撑多久，减少“快用完了才发现”的被动情况。
  - 建议：第一版优先走后端按次记录 + admin-web 统计页，先覆盖总调用量、按 Key 用量、按接口类型拆分和按天趋势，不提前做复杂告警系统。
