...
- `COS_BUCKET` 可以写“桶名称”本体，例如 `car-platform`，此时需要同时配置 `COS_APP_ID`
- 也兼容直接写完整 bucket 名称，例如 `car-platform-1250000000`，这种写法可省略 `COS_APP_ID`
...
- 旧的 `POST /app/valuation-orders/photos` 本地静态目录上传入口视为停用，不再作为新链路的一部分
- 上传票据只允许图片类型，避免把任意文件对象写进车辆照片目录
